...
bert_path = '../model/uncased_L-12_H-768_A-12/'
root_path = '.'
...
flags.DEFINE_string("gcp_project", None, "gcp project")
...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        if os.path.exists(os.path.join(FLAGS.data_dir, 'train.txt')):
            labels = set(["O", "[CLS]", "[SEP]", "X"])
            ...
            return ["O", "B-AS", "I-AS", "B-OP", "I-OP", "X", "[CLS]", "[SEP]"]
...
#         raise ValueError("At least one of `do_train` or `do_eval` must be True.")
    ...
    if FLAGS.do_predict:
        ...
        def result_to_pair(writer):
            token_lines = open(token_path).read().splitlines()
            tokens = [[]]
            for line in token_lines:
                tokens[-1].append(line)
                if id2label[int(line)] == '[SEP]':
                    tokens.append([])
            tokens.pop()

            for predict_line, prediction, token_list in zip(predict_examples, result, tokens):
                idx = 0
                line = ''
                ...
                for token_id, id in enumerate(prediction):
                    if idx >= len_seq:
                        break
                    if id == 0:
                        continue
                    ...
                    # skip character-level features
                    if id2label[int(token_list[token_id])] == 'X':
                        continue
